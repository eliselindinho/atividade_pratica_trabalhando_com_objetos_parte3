...
let listarUsuariosNaTabela = (filtrados = undefined) => {
  //Função que exibe tabela no DOM
  ...
  lista.forEach((usuario) => {
    //Percorre array lista e devolve template string com tags html com cada propriedade do objeto que foi passada
  ...
  if (lista.length === 0) {
    //Testa se o cumprimento do array lista é vazio, e caso seja, insere no html a template string abaixo;
    elementoTBody.innerHTML = `<tr><td colspan="5">Nenhum dado encontrado</td></tr>`;
  }
};

let filtraDados = () => {
  //Função que filtra os dados solicitados;
  let filtrados = []; //Variável que recebe um array vazio;

  if (pesquisa.value.toLowerCase() !== "") {
    //Verifica se o valor do input (convertido já em minúsculo) é diferente de vazio;
    filtrados = listaUsuarios.filter(
      //Atribui à variável filtrados um método que vai filtrar um dado de acordo com as condições abaixo;
      (usuario) =>
    ...
    //Varificação para caso a condição acima não seja atendida, a variável filtrados vai receber apenas a variável listaUsuarios
...
let limpaFormulario = () => {
  //Função para limpar o que foi digitado no input;
